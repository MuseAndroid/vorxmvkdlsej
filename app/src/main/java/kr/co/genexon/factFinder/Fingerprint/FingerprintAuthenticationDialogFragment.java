...
import kr.co.genexon.factFinder.LoginActivity;
...
public class FingerprintAuthenticationDialogFragment extends DialogFragment
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences loginStatus = getActivity().getSharedPreferences("loginStatus", MODE_PRIVATE);
        screenStatus = loginStatus.getString("loginScreen", "");
    }
...
//        Log.d("FactFinder", "저장된 값 = " + mSharedPreferences.getString(Constant.PASSWORD_NUN, "0000"));
        switch (screenStatus) {
            case "기업고객" :
                SharedPreferences comPref = getActivity().getSharedPreferences("compLogin", MODE_PRIVATE);
                savedPassword = comPref.getString("cUserPw", "");
                if (mPassword.getText().toString().equals(savedPassword)) {
                    dismiss();
                ...
                break;
            case "개인고객" :
                SharedPreferences normalPref = getActivity().getSharedPreferences("userLogin", MODE_PRIVATE);
                savedPassword = normalPref.getString("userPw", "");
                if (mPassword.getText().toString().equals(savedPassword)) {
                    dismiss();
                    secretAuthorize.success();
                    mStage = Constant.FINGERPRINT;
